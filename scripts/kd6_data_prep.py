...
os.makedirs("kd6_feats",exist_ok=True)
buf_f,buf_t=[],[]; shard=0; hours=0.0; n=0; bysrc={}
def flush():
    global shard,buf_f,buf_t
    if not buf_f: return
    np.savez(f"kd6_feats/shard_{shard:03d}.npz",eo=np.array(buf_f,dtype=object),txt=np.array(buf_t,dtype=object))
...
for mani,src in [("yt_manifest.tsv","youtube"),("ivod_manifest.tsv","ivod")]:
...
flush()
print(f"DONE kd6: {n} clips, {hours:.1f}h Taiwan -> {shard} shards | by source: {bysrc}",flush=True)
