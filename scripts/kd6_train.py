...
F=[]; Y=[]
for sh in sorted(glob.glob("kd6_feats/shard_*.npz")):
...
for step in range(1,STEPS+1):
    ...
    if step%EVAL_EVERY==0:
        ...
        if rc<best: best=rc; torch.save({"model":m.state_dict()},"ft_runs/kd6_best.pt")
        print(f"  [eval@{step}] recognition {rc:.4f} | raw-Traditional {rtc:.4f}{tag}",flush=True)
print(f"\n=== KD6 (Taiwan-at-scale): best recognition CER {best:.4f} vs warm 0.108 / X-ASR 0.064 (Taiwan-only, no Mainland) ===",flush=True)
